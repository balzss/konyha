...
import { Menu, MenuItem, Divider, ListItemIcon, Checkbox } from '@mui/material';
import { Add as AddIcon, Search as SearchIcon, Tune as TuneIcon } from '@mui/icons-material';
...
export default function HomeTopBar() {
  ...
  const title = selectedTags.length
    ? selectedTags.map((tagSlug) => tags?.find(({ slug }) => tagSlug === slug)?.name).join(', ')
    : 'All recipes';
  ...
  const handleSelectTag = (tagSlug: string) => {
    const newSelection = selectedTags.includes(tagSlug)
    ...
    <>
      ...
      >
        <MenuItem onClick={(e) => router.push('/')}>
          <ListItemIcon>
            <Checkbox edge="start" checked={!selectedTags.length} tabIndex={-1} disableRipple />
          </ListItemIcon>
          All recipes
        </MenuItem>
        {tags &&
          tags.length > 0 && [
            ...
            ...tags.map(({ name, slug }) => (
              <MenuItem key={slug} onClick={() => handleSelectTag(slug)}>
                <ListItemIcon>
                  <Checkbox edge="start" checked={selectedTags.includes(slug)} tabIndex={-1} disableRipple />
                </ListItemIcon>
                {name}
              </MenuItem>
            )),
